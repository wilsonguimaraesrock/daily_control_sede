-- FIX ASSESSORA ADM TASK VISIBILITY - CORRECTED VERSION
-- Date: 2025-01-08
-- Problem: Assessora ADM can only see her own tasks, not tasks from other assessoras
-- Fix: Replace "current_user" reserved word with valid alias
  ...
  USING (
    ...
    (
      EXISTS (
        ...
        WHERE up_current.user_id = auth.uid() 
        AND up_current.role = 'assessora_adm'
        AND up_current.is_active = true
      )
      AND
      EXISTS (
        SELECT 1 FROM public.user_profiles up_creator
        WHERE up_creator.user_id = created_by 
        AND up_creator.role = 'assessora_adm'
        AND up_creator.is_active = true
      )
    )
    OR
